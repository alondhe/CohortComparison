...
getChartData <- function(connectionDetails, 
  ...
  sql <- SqlRender::loadRenderTranslateSql(sqlFilename = "generateComparisonSummary.sql", 
  ...
  data$ABS_STANDARD_DIFF <- abs((data$TARGET_STAT_VALUE - data$COMPARATOR_STAT_VALUE) / 
...
plotlyXy <- function(data, 
  ...
  pal <- c("#e0f3f8", "#ff5c33", "#fc8d59", "#1b7837", 
           ...
           "#ffbcec", "#f442dc", "#ff0000")
  pal <- setNames(pal, c("Balanced", "Measurement", "Race", "Drug",
                         "Condition", "Metadata", "Procedure", "Device", 
                         "Observation", "Ethnicity", "Gender"))
  ...
  p <- plotly::plot_ly(data = data, x = ~COMPARATOR_STAT_VALUE, y = ~TARGET_STAT_VALUE, 
    ...
    plotly::layout(font = globalFont,
                   ...
                   xaxis = list(titlefont = list(size = 22), 
                                ...
                                title = OhdsiRTools::getCohortDefinitionName(baseUrl = baseUrl,
